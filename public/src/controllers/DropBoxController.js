class DropBoxController{
    ...
    constructor(){
        ...
        this.listFileEl = document.getElementById("list-of-files-and-directories");
    ...
    connectFireBase(){
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyC8zw4o_DnEvcOg7FXid2b0qCByzeX8Oag",
            authDomain: "dropbox-clone-2961f.firebaseapp.com",
            databaseURL: "https://dropbox-clone-2961f.firebaseio.com",
            projectId: "dropbox-clone-2961f",
            storageBucket: "gs://dropbox-clone-2961f.appspot.com",
            messagingSenderId: "480228442431",
            appId: "1:480228442431:web:1b922d38e4fb1a7a"
        };
        // Initialize Firebase
    ...
    initEvents(){
        ...
        this.btnRename.addEventListener("click", event=>{
            let li = this.getSelecions()[0];
            let file = JSON.parse(li.dataset.file);
            let name = prompt("renomea o arquivo: ",file.name);
            if(name){
                file.name = name;
        ...
        this.listFileEl.addEventListener("onselectionchange", e=>{
            switch(this.getSelecions().length){
                case 0:
    ...
    initEventsLi(li){
        ...
        li.addEventListener('click', e=>{
            if(e.shiftKey){
                ...
                //verifica se existe a li
                //declara variaveis para saber o inicio e o fim
                let indexStart;
                let indexEnd;
                let lis = li.parentElement.childNodes;
                if(firstLi){
                    //pega todos elementos para comparar os indexs
                    lis.forEach((el, index)=>{
                        if(firstLi === el) indexStart = index;
                        else if(li === el) indexEnd = index;
                    });
                    ...
                    //metodo para disparar um evento
                    this.listFileEl.dispatchEvent(this.onselectionchange);
                    //para ele nao continuar a executar o codigo o toggle
                    return true;
                }
            }
            if(!e.ctrlKey){
    ...
    readFiles(){
        ...
        this.getFireBaseRef().on('value',snapshot=>{
            ...
            snapshot.forEach(snapshotItem=>{
                if(snapshotItem.val().type){
                    this.listFileEl.appendChild(this.getFileView(snapshotItem.val(), snapshotItem.key));
                }
            });
        });
    }
